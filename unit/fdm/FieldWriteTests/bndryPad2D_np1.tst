...
def main():
    ...
    # Run the unit test
    filename = "field.bin"
    runCommand(("mpiexec -np %d ../testField --bndryPad=1,1"
                + " --writeBndryPad --writeto=%s") % (options.numproc, filename), True)

    # Read the generated output and compare it to the reference values
    if numpy:
        print
        print "Reading and verifying the Field ...",
        ref = numpy.arange(100,dtype="d").reshape((10,10))
        f = numpy.fromfile(filename).reshape((10,10))
        assert((f[1:-1,1:-1] == ref[1:-1,1:-1]).all())
        assert((f[ 0,:] == -1).all())
        assert((f[-1,:] == -1).all())
        assert((f[:, 0] == -1).all())
        assert((f[:,-1] == -1).all())
    ...
    print
    print "-----------------------------------------------------------------"
    runCommand(("mpiexec -np %d ../testField --commPads=1,1"
                + " --bndryPad=1,1 --writeBndryPad --writeto=%s") %
