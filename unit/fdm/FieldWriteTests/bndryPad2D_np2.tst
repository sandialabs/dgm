...
def main():
    ...
    # Run the unit test
    filename = "field.bin"
    print "options.numproc =", options.numproc
    runCommand(("mpiexec -np %d ../testField --bndryPad=2,2"
    ...
    if numpy:
        ...
        ref = numpy.arange(144,dtype="d").reshape((12,12))
        f = numpy.fromfile(filename).reshape((12,12))
        assert((f[2:-2,2:-2] == ref[2:-2,2:-2]).all())
        assert((f[: 2,:] == -1).all())
        assert((f[-2:,:] == -1).all())
        assert((f[:,: 2] == -1).all())
        assert((f[:,-2:] == -1).all())
    ...
    runCommand(("mpiexec -np %d ../testField --commPads=2,2"
                + " --bndryPad=2,2 --writeBndryPad --writeto=%s") %
