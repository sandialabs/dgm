...
def main():
    ...
    runCommand(("mpiexec -np %d ../testField --dims=4,4,4"
                + " --bndryPad=2,2,2 --writeBndryPad --writeto=%s") %
    ...
    if numpy:
        ...
        f = numpy.fromfile(filename).reshape((8,8,8))
        assert((f[2:-2,2:-2,2:-2] == ref[2:-2,2:-2,2:-2]).all())
        assert((f[: 2,:,:] == -1).all())
        assert((f[-2:,:,:] == -1).all())
        assert((f[:,: 2,:] == -1).all())
        assert((f[:,-2:,:] == -1).all())
    ...
    print
    print "-----------------------------------------------------------------"
    runCommand(("mpiexec -np %d ../testField --dims=4,4,4 --commPads=2,2,2 " +
                "--bndryPad=2,2,2 --writeBndryPad --writeto=%s") %
               (options.numproc, filename), True)
