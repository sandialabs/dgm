...
# ==============================================================================
class dgm_test_support:
  """Class to help support dgm tests"""
  def __init__( self, description = 'Dgm testing script.', \
    ...
    p.add_option("", "--computer", dest="computer", default=None, \
                     action="store", type="string", help="Computer to run on")
  ...
  def shell(self, cmd, logfile=None, ignore_status=False):
    status = 0
  ...
  def call(self, cmd, logfile=None, ignore_status=False):
    ...
    # on mutrino cray, replace mpiexec with srun
    if self.opts.cray == True:
      if (cmd.find('mpiexec') == -1):
        ...
        if (cmd.find('env') != -1):
          ...
          cmd = cmd[0:new_cmd+1] + 'srun ' + cmd[new_cmd+1:]
        else:
  ...
  def dgm_serial(self, root, args=''):
    status = 0
    log = '%s.log' % (root)
    cmd = 'dgm.exe %s %s' % (args,root)
    status += self.call(cmd, log)
    return status

  # ignore sleep on non-cray systems
  def sleep(self, s):
    status = 0
    if self.opts.cray == True:
      time.sleep(s)
  ...
  def dgm(self, root, args=''):
    status = 0
    log = '%s.log' % (root)
    cmd = self.wrap_cmd('dgm', root, self.opts.nprocs, args)
    status += self.call(cmd, log)
    return status

  def dgm_diff_serial(self, aeps, reps, ref, test, root):
    status = 0
    log = '%s.log' % (root)
    cmd = 'dgm_diff.exe -aeps %g -reps %g -r1 %s.ref -r2 %s.rst %s'%(aeps,reps,ref,test,root)
    status += self.call(cmd, log)
    return status

  def dgm_diff(self, aeps, reps, ref, test, root):
    status = 0
    log = '%s.log' % (root)
    cmd = self.wrap_cmd('dgm_diff',root,self.opts.nprocs, \
        '-aeps %g -reps %g -r1 %s.ref -r2 %s.rst'%(aeps,reps,ref,test))
    status += self.call(cmd, log)
    return status

  def dgm_opt(self, root, args=''):
    status = 0
    log = '%s.log' % (root)
    cmd = self.wrap_cmd('dgm_opt', root, self.opts.nprocs, args);
    status += self.call(cmd, log)
    return status

  def dgm_clean(self, root):
    status = self.call('dgm_clean %s'%root)
    return status

  def dgm_peopt(self, root, args=''):
    status = 0
    log = '%s.log' % (root)
    cmd = self.wrap_cmd('dgm_peopt', root, self.opts.nprocs, args);
    status += self.call(cmd, log)
    return status
