...
default=False,help="parse a root.inp file");
...
if args.input:
  ...
  text = file.read();
  print text
  print 'root.inp file parsing not yet implemented'
  sys.exit()
else:
  file = open(json_fname); 
  text = file.read()
  text = re.sub(r"([^/])//[^/].*",r"\1",text)            # C++ //  comments
  text = re.sub(r"/\*.*?\*/", "", text, flags=re.DOTALL) # C /* */ comments
