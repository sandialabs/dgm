...
root = 'hnode3dspecthextet'      # root filename for test
...
if dts.opts.diff and not status:
  ...
  status += dts.call('pdgm_diff.exe -aeps %g -reps %g -r1 %s.ref -r2 %s.rst %s' % (aeps,reps,root,root,root))

# ------------------------------
if dts.opts.baseline and not status:
  if dts.opts.verbose: print '---> Baseline %s' % (root)
  status += dts.call('cp hnode3dspecthextet.rst hnode3dspecthextet.ref')
