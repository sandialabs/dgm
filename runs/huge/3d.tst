...
root = '3d'       # root filename for test
...
if dts.opts.preprocess and not status:
  ...
  status += dts.call('dgm_mesh.exe -ascii 0 -binary 1 -tecplot 0 -r 3d -nograph -nsd 3 -nx 128 -ny 128 -nz 128')
...
if dts.opts.baseline and not status:
  ...
  status += dts.call('cp 3d.rst 3d.ref')
...
if dts.opts.clean and not status:
  ...
  status += dts.call('rm 3d.grd 3d.cn')
