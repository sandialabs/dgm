...
Value::CZString::CZString() : cstr_(0), index_(0)
{
}
...
{
   if ( cstr_ )
#ifdef JSONCPP_CASE_SENSITIVE
      return strcmp( cstr_, other.cstr_ ) < 0;
#else
      return strcasecmp( cstr_, other.cstr_ ) < 0;
#endif
...
Value &
Value::operator[]( unsigned long long index )
{
   JSON_ASSERT( index >= 0 );
   return (*this)[ ArrayIndex(index) ];
}
...
const Value &
Value::operator[]( unsigned long index ) const
{
   JSON_ASSERT( index >= 0 );
   return (*this)[ ArrayIndex(index) ];
}
...
const Value &
Value::operator[]( unsigned long long index ) const
{
   JSON_ASSERT( index >= 0 );
   return (*this)[ ArrayIndex(index) ];
}
...
/** Since this is a non-const member function, it calls a different
    version of operator[key] that inserts a new null entry **/
Value
Value::set( const char *key,
            const Value &defaultValue )
{
  //std::cout << defaultValue.type() << " " << defaultValue;
  const Value *value = &((*this)[key]);
  //std::cout << key << " : " << (value == &null ? defaultValue : *value);
...
Value::set( const std::string &key,
            const Value &defaultValue )
{
   return set( key.c_str(), defaultValue );
}
