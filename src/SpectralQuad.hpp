#ifndef DGM_SPECTRALQUAD_HPP
#define DGM_SPECTRALQUAD_HPP

/** \file SpectralQuad.hpp
    \brief SpectralQuad Element declaration
    \author Scott Collis
    \author James Overfelt
    \author Thomas Smith

    SpectralQuad is a four node quadralateral element that supports both
...
/// SpectralQuadrilateral Element class
class SpectralQuad : public BaseQuad {

  typedef GeometryDBSetApprox GeometryDB;  ///< Type of GeometryDB to be used
 ...
 public:
  ...
  /// Constructor
  SpectralQuad(const Ordinal, const Ordinal, const vector<Point>&,
  ...
  /// Copy constructor
  SpectralQuad(const SpectralQuad*);
  ...
  /// 3d local gradient not defined for SpectralQuad
  ...
  /// Clone this element
  Element* clone() const {return new SpectralQuad(this);}
...
protected:
  ...
  /** \note We have to update all data or pointers, which were not serialized.
      this can not be done in this constructor, since the data is not there
      yet. */
  SpectralQuad() {};
#else
protected:
...
/// Output a SpectralQuad Element in DGM mesh file format
inline void SpectralQuad::write_mesh(ostream &f, const unsigned prec) const {
  f << "ELEMENT " << gid() << "  SPECTRALQUAD\n";
  f << scientific;
  f.precision(prec);
...
/// Allocate memory for the SpectralQuad workspace
inline void SpectralQuad::make_workspace(const Ordinal qtot,
