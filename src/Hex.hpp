...
/** \file  Hex.hpp
    \brief Hexahedral element declaration
    \author Scott Collis
*/
...
public:
  ...
  /// Constructor
  Hex(const Ordinal, const Ordinal, const Ordinal, const vector<Point>&);
  ...
  Scalar inner_product(dVector &, const Ordinal) const;
  ...
  //@{
  /// 1d gradient undefined for Hex
  void gradient(const dVector &, dVector &) const {DGM_UNDEFINED;}
  /// 2d gradient undefined for Hex
  ...
  /// 2d local gradient undefined for Hex
  ...
  virtual Topology::ElementType element_type() const {return Topology::Hex;}
  ...
  Element* clone() const {return new Hex(this);}
...
private:
  ...
  void get_flux(dVector &, const int, const Ordinal, const Element *const,
                const char) const;
...
/// output a Hex Element in DGM mesh file format
inline void Hex::write_mesh(ostream &f, const unsigned prec) const {
  f << "ELEMENT " << gid() << "  HEX\n";
  f << scientific;
  f.precision(prec);
...
/// allocate memory for the Hex workspace
inline void Hex::make_workspace(const Ordinal qtot, const Ordinal nModes) {
  if (qtot > Qmax) {
    Qmax = qtot;
