/** \file  Line.cpp
    \brief Line element implementation
    \author Scott Collis
*/
...
BOOST_CLASS_EXPORT_GUID(DGM::Line, "DGM::Line");
#endif

namespace DGM {

// Define static storage
BasisDB Line::bdb;                   // define Basis database
Line::GeometryDB Line::gdb("Line");  // define Geometry database
...
/// Primary constructor
Line::Line(const Ordinal ID, const Ordinal P, const Ordinal Q,
           const vector<Point> &X) : BaseLine(ID,Basis::Ortho) {
  // cout << "Line constructor, eid = " << ID << endl;
  nVerts(2);
  nEdges(2);
  nFaces(0);
  nSides(2);
  ...
  // there is only one "quadrature" point on a 1d "edge"
  int qedg = 1;
  edge.reserve(nEdges());
...
Line::Line(const Line *E) {
  // cout << "Line copy constructor for Eid = " << E->id << endl;
...
/// Destructor
Line::~Line() {
  //cout << "~Line: eid= "<<id<<endl;
}

Basis* Line::get_basis(const int der=0) const {
  Basis* b;
  char buf[128];
  sprintf(buf,"%lld_%lld_%d_%lld_%lld_%lld",
          (long long)type,(long long)L(),der,
...
void Line::get_mass_matrix() {
  // cout << "Line::get_mass_matrix for element " << id << endl;
#ifdef DGM_BASIS_NORM
  M.reset( new Minfo );
  M->mat.resize(1,nModes());
  const Ordinal i=0;
  for (Ordinal j=0; j<nModes(); j++) M->mat[i][j] = 1.0/geom->J[i];
#else
  M.reset( new Minfo );
  M->mat.resize(1,nModes());
  dVector f(qtot);
  ...
  for (int j=0; j<nModes(); j++) M->mat[0][j] = 1.0/M->mat[0][j];
#endif
...
#ifdef LINE_USE_CBLAS
  for (int i=0; i<nModes(); i++) {
    std::copy( phi[i], phi[i]+phi.cols(), &wk[0] );  // fill wk
    dvmul( J, wk );                   // wk *= J
    dvmul( w, wk );                   // wk *= w
    mvmult( phi, wk, M->mat.row(i) ); // M[i] = phi[i][q] * u[q]
  }
#else
  for (int i=0; i<nModes(); i++) {
...
void Line::get_mass_matrix(const dVector &x) {
  //cout << "Line::get_mass_matrix(x) for element " << id << endl;
...
void Line::solve_mass_matrix(dVector &fh) const {
  if (full_mass) {
#ifdef DGM_CHOLESKY_MASS_MATRIX
...
#ifdef DGM_BASIS_NORM
    const Ordinal j=0;
    for (Ordinal i=0; i<nModes(); i++) fh[i] *= M->mat[j][i];
#else
    LU_solve(M->mat, M->pivots, fh);
#endif
  }
...
void Line::normal_times_gradient_side(
    const dVector &f,
    const int sid,
    dVector &fsum ) const {
  const int nsd = 1;
...
void Line::get_local_mass_matrix(const dVector &factor) {
  //cout << "Line::get_local_mass_matrix(x) for element " << id << endl;
  // WARNING:  might be dangerous to reset the mass matrix to this new value
...
void Line::get_local_derivative_matrix() {
  // cout << "Line::get_local_derivative_matrix for element " << id << endl;
...
void Line::get_local_derivative_matrix(const dVector & factor) {
  // cout << "Line::get_local_derivative_matrix for element " << id << endl;
  Dxloc.reset( new Minfo );
  Dxtloc.reset( new Minfo );
  Dxloc->mat.resize(nModes(),nModes());  // pivots untouched
...
void Line::get_local_laplace_matrix() {
  // cout << "Line::get_local_derivative_matrix for element " << id << endl;
...
void Line::get_local_laplace_matrix(const dVector & factor) {
  // cout << "Line::get_local_derivative_matrix for element " << id << endl;
  DDloc.reset( new Minfo );
  DDloc->mat.resize(nModes(),nModes()); // pivots untouched
  // work vectors
...
void Line::fill(string function_string, Operation operation) {
  state = Physical;
  parser_init();
...
void Line::fill(Mode &v) {
  //cout << "Line::fill(Mode &)" << endl;
...
void Line::fill(Mode &v, dVector &f) const {
  //cout << "Line::fill(Mode &, dVector &) const" << endl;
  for (Ordinal i=0; i<qa; i++)
    f[i] = v.a[i];
}
...
void Line::random() {
  // cout << "Line::random()" << endl;
  state = Transform;
  for (Ordinal i=0; i<L(); i++) uh[i] = Element::random(-one,one);
  backward_transform();
}

void Line::inner_product() {
  //  cout << "Line::inner_product for Eid = " << id << endl;
  inner_product(u, uh);
  state = Transform;
}

void Line::inner_product(dVector &f, dVector &fh) const {
  dMatrix &phi = psi->Ba;
...
void Line::forward_transform() {
  if (state == Transform) {
    cerr << "Line::forward_transform -- already in transform space" << endl;
    DGM::Comm::World->exit(DGM::FAILURE);
  }
...
void Line::forward_transform(dVector &f, dVector &fh) const {
  inner_product(f,fh);
  solve_mass_matrix(fh);
}

void Line::forward_transform(Element *F, Element *) {
  //cout << "Line::forward_transform(*F,*)" << endl;
  if (state == Transform) {
    cerr << "Line::transform -- already in transform space"<<endl;
    DGM::Comm::World->exit(DGM::FAILURE);
  }
  inner_product(u,uh);
...
void Line::fill_sides(const Scalar f, const Operation operation) {
  switch (operation) {
  case Equal:
    edge[0].u[0] = f;
    edge[1].u[0] = f;
  ...
  case Divide:
    ...
    break;
  default:
    throw DGM::exception("Illegal operation in Line::fill_sides");
    break;
  }
}
...
/** For the line element, if we use the Gauss-Lobatto over the element
    then the values at the edges are directly available. */
void Line::fill_sides(const dVector &f) {
  edge[0].u[0] = f[0];
  edge[1].u[0] = f[qa-1];
}

// flux version
void Line::fill_sides(const dVector &f, const dVector &) {
...
void Line::load_side(const dVector &f, const Ordinal side_number, dVector &g)
  const {
  if(side_number > 1) {
    DGM::Comm::World->cerr() << "Line::side number larger than 1"<<endl;
    DGM::Comm::World->exit();
  }
  if(side_number == 0)
...
void Line::backward_transform() {
  if (state == Physical) {
    DGM::Comm::World->cerr() << "Line::backward_transform -- already in "
      "physical space"<<endl;
    DGM::Comm::World->exit();
  }
...
// To do this in-place requires scratch space
void Line::backward_transform(dVector &fh, dVector &f) const {
  dMatrix &phi = psi->Ba;
#ifdef LINE_USE_CBLAS
...
/** For the line element, if we use the Gauss-Lobatto over the element
    then the values at the edges are directly available. */
void Line::get_side_coordinates(const Ordinal endid, Point *pend) const {
  switch(endid) {
    case 0: {
      pend[0] = Point(vert[0]->x);
      break;
    }
    case 1: {
      pend[0] = Point(vert[1]->x);
      break;
    }
    default: {
      cerr << "Line::get_side_coordinates illegal end id." << endl;
      DGM::Comm::World->exit();
    }
  }
}

void Line::compute_geometry() {
...
Element::StabMetrics Line::compute_metrics() {
  return compute_metrics_1d();
}

/// Computes geometric factors for the Element Sides
void Line::compute_side_geometry() {
  //  cout << "Line::compute_side_geometry(), Eid = "<< id << endl;
...
void Line::add_Dirac_delta(const Ordinal qpt, const Scalar amp) {
...
/** Add a discrete version of a Dirac delta function at an arbitrary point p
    with amplitude equal amp.  p must be within the Line element. */
void Line::add_Dirac_delta(const Point &p, const Scalar amp) {
  Scalar r;
  // First get the local coordinate on line
  if(!get_local_coordinates(p,r)) {
    DGM::Comm::World->cerr()<<
      "Line::Add_Dirac_delta: could not find (r)"<<endl;
    DGM::Comm::World->exit(DGM::FAILURE);
  }
  const Point q(r);
  add_local_Dirac_delta(q,amp);
}

void Line::add_local_Dirac_delta(const Point &p, const Scalar amp) {
  const Scalar r=p.x;
  dMatrix im;
...
void Line::add_local_Dirac_delta(const vector<dMatrix> &M, const Scalar amp) {
  if (1!=M.size())
    error("Line::add_local_Dirac_delta: Incorrect matrix size.");
  if (M[0].cols() != qa)
    error("Line::add_local_Dirac_delta: Inconsistant matrix size.");
...
Scalar Line::interpolate_point(const Point &p) const {
  Scalar r;
...
/// Extract the solution at the Point p
Scalar Line::interpolate_local_point(const Point &p) const {
  const Scalar r=p.x;
  dMatrix im;
...
void Line::interpolate_points(const vector<Point> &ps, dVector &vals) const {
  if (state != Physical) {
    cerr << "Line::interpolate_points -- not in physical space"<<endl;
    DGM::Comm::World->exit(DGM::FAILURE);
  }
...
void Line::lift (const Element *const F, dVector &in, const char dir) const {
...
void Line::lift(dVector &in) const {
  const Element *const U = this;
  const dVector &wa = U->psi->wa;
  const dVector &J  = geom->J;
  in[0]    += U->edge[0].u[0] / ( wa[0] * J[0] );
  in[qa-1] += U->edge[1].u[0] / ( wa[qa-1] * J[qa-1] );
}

// Only one quadrature point per side, no summation required
Scalar Line::compute_side_integral( dVector &f, Ordinal sid ) const {
  return f[0];
}
Scalar Line::compute_side_measure() const { return 1.0; }

// Collocation derivative matrix
void Line::get_derivative_matrix() {
  D.reset (new Dinfo);
  D->Da.resize(qa,qa);
  D->Dat.resize(qa,qa);
...
void Line::gradient(const dVector &f, dVector &fx ) const {
  gradient(f,fx,fx);
}

void Line::gradient(const dVector &f, dVector &fx, dVector &) const {
  dVector fr(g1_wk,qtot);
  mvmult( D->Da, f, fr );
  dvmul( geom->rx, fr, fx );
}

void Line::gradient(const dVector &f, dVector &df, char dir) const {
...
/// Computes the operator \f$(\vec f \cdot \nabla)\f$
/** We really compute \f$\int_\Omega f \nabla g \f$ which if discretized is
    \f$ f^T M R_x D g \f$ where \f$M\f$ is a diagonal matrix with quadrature
    weights and determinate of the Jacobian, \f$R_x\f$ is a diagonal matrix
    with the metric of the transformation to the master element and \f$D\f$
    is the collocation derivative matrix.

    Taking the transpose leads to \f$ g^T M M^{-1} D^t R_x^T M^T f \f$
    so that we need to compute \f$ M^{-1} D^t R_x M f \f$.   */
void Line::fdotgrad(const dVector &f, const dVector &, dVector &fx) const {
  dVector fr(g1_wk,qtot);
  dVector &wa = psi->wa;
  dVector &J = geom->J;
  dvmul( wa, f, fr );
...
void Line::output(FILE *fp, const char flag) const {
  if (state == Transform)
  ...
  case 'f':  // Tecplot footer
    break;
  default:
    throw DGM::exception("Illegal flag in Line::output, flag = "+asString(flag));
  }
}

// norms and errors

Scalar Line::norm_Linf() const {
  return norm_inf(u);
}

void Line::norm_L2(Scalar *L2, Scalar *area) const {
  dVector &wa = psi->wa;
  dVector &J = geom->J;
  dVector b(qtot);
  dVector f(qtot);
...
void Line::norm_L2(const dVector& lv, Scalar *L2, Scalar *area) const {
  dVector &wa = psi->wa;
  dVector &J = geom->J;
  dVector b(qtot);
  dVector f(qtot);
...
Scalar Line::norm_L2(const dVector& lv) const {
  Scalar L2, area;
  norm_L2( lv, &L2, &area );
  return sqrt(L2/area);
}
...
void Line::norm_H1(Scalar *H1, Scalar *area) const {
  dVector &wa = psi->wa;
  dVector &J = geom->J;
...
Scalar Line::norm_H1() const {
  Scalar H1, area;
  norm_H1( &H1, &area );
  return sqrt(H1/area);
}

Scalar Line::integrate(Scalar *area) const {
#ifdef DGM_USE_WJ
...
Scalar Line::integrate(const dVector &v, Scalar *area) const {
#ifdef DGM_USE_WJ
  if (area!=NULL) *area = sum( geom->wJ );
...
void Line::set(const dVector &fh, const Ordinal nModes2) {
  // cout << "Line::set(const dVector &, const int)" << endl;
...
Scalar Line::compute_measure() const {
  dVector &wa = psi->wa;
  dVector &J = geom->J;
  dVector b(qtot);
  dVector f(qtot);
...
Scalar Line::L2_inner_product(const Element *el, Scalar *length) const {
  dVector &wa = psi->wa;
  dVector &J = geom->J;
  dVector b(qtot);
  dVector f(qtot);
...
Scalar Line::L2_inner_product(const dVector& vh, const dVector& wh,
                              Scalar *length) const {
  dVector &wa = psi->wa;
  dVector &J = geom->J;
  dVector b(qtot);
...
void Line::get_side_Ds(const int, const dVector& , dVector& ) const {
  cerr << "Line::get_side_Ds(const int, const dVector& ,dVector&) undefined"
       << endl;
  DGM::Comm::World->exit(DGM::FAILURE);
}

void Line::get_side_curvature( const int sid, dVector &dcurv) const {
  cerr << "Line::get_side_curvature(const int, dVector& ) undefined" << endl;
  DGM::Comm::World->exit(DGM::FAILURE);
}

void Line::make_constant() {
...
void Line::low_modes(const Ordinal np) {
...
void Line::coarse_scales(const int p_inc) {
  // cout << "Line::coarse_scales(const int p)" <<endl;
  bool needs_transform = false;
  if (state == Physical) {
    forward_transform();
    needs_transform = true;
...
void Line::coarse_scales(const int p_inc, dVector &vh) {
  // cout << "Line::coarse_scales(const int p, dVector &vh)" <<endl;
  for (Ordinal i=L()-p_inc; i<L(); i++) vh[i] = 0.0;  // zero the high modes
}

void Line::fine_scales(const int p_inc) {
  // cout << "Line::fine_scales(const int p)" <<endl;
  bool needs_transform = false;
  if (state == Physical) {
    forward_transform();
    needs_transform = true;
...
void Line::fine_scales(const int p_inc, dVector &vh) {
  // cout << "Line::fine_scales(const int p, dVector &vh)" <<endl;
  for (Ordinal i=0; i<L()-p_inc; i++) {
    vh[i] = 0.0;
  }
}

void Line::fine_scales_transform(const int p_inc, dVector &vh) const {
  // cout << "Line::fine_scales_transform(const int p, dVector &vh)" <<endl;
...
void Line::fill_basis_shift_matrix(dMatrix &G) const {
  const int nModes = 2;
  const int Nvert = 2;
