#ifndef DGM_SPECTRAL_LINE_HPP
#define DGM_SPECTRAL_LINE_HPP

/** \file  SpectralLine.hpp
    \brief SpectralLine element declaration. */
...
/// One-dimensional SpectralLine Element class
class SpectralLine : public BaseLine {

  typedef GeometryDBSetApprox GeometryDB;  ///< Type GeometryDB to be used
 ...
 public:
  ...
  /// Constructor
  SpectralLine(const Ordinal, const Ordinal, const vector<Point>&);

  /// Copy constructor
  SpectralLine(const SpectralLine*);
  ...
  Element* clone() const {return new SpectralLine(this);}
...
/// Output a SpectralLine Element in DGM mesh file format
inline void SpectralLine::write_mesh(ostream &f, const unsigned prec) const {
  f << "ELEMENT " << gid() << "  LINE\n";
  f << scientific;
  f.precision(prec);
...
/// Allocate memory for the SpectralLine workspace
inline void SpectralLine::make_workspace(const Ordinal qtot,
